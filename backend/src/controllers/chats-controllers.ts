...
import { configOpenAI } from "../config/openai-config";
  ...
  try {
   ...
   console.log("OpenAI API Response:", chatCompletion);
    ...
    if (chatCompletion.choices && chatCompletion.choices.length > 0) {
      ...
      user.chats.push(userMessage, assistantMessage); 
...
export const sendChatsToUser = async (
  ...
  try {
    ...
    console.log(error);
    return res.status(200).json({ message: "ERRORR", cause: error.message });
  }
};
